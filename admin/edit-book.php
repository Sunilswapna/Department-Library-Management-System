...
if(strlen($_SESSION['alogin']) == 0) {   
    ...
    if(isset($_POST['update'])) {
        ...
        $isbn = $_POST['isbn'];
        $price = $_POST['price'];
        $Copies = $_POST['copies'];  // Use POST here
        $bookid = intval($_GET['bookid']);

        $sql = "UPDATE tblbooks SET BookName=:bookname,AuthorsName=:aname, CatId=:category, AuthorId=:author, ISBNNumber=:isbn, BookPrice=:price, Copies=:Copies WHERE id=:bookid";
        ...
        $query->bindParam(':Copies', $Copies, PDO::PARAM_STR);  // Ensure this matches
        $query->bindParam(':bookid', $bookid, PDO::PARAM_INT);    // Using PDO::PARAM_INT for IDs
                    ...
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <form role="form" method="post">
                                <?php 
                                $bookid = intval($_GET['bookid']);
                                $sql = "SELECT tblbooks.BookName, tblcategory.CategoryName, tblbooks.AuthorsName, tblbooks.Copies, tblcategory.id as cid, tblauthors.AuthorName, tblauthors.id as athrid, tblbooks.ISBNNumber, tblbooks.BookPrice, tblbooks.id as bookid 
                                        FROM tblbooks 
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="text" name="aname" value="<?php echo htmlentities($result->AuthorsName); ?>" readonly />
                                </div>
...
<div class="form-group">
    <label>Publication<span style="color:red;">*</span></label>
    <input type="text" class="form-control" value="<?php echo htmlentities($result->AuthorName); ?>" readonly>
    <input type="hidden" name="author" value="<?php echo htmlentities($result->athrid); ?>">
</div>
